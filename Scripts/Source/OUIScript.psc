...
Form property AmuletOfMara auto
...
bool orcache_locked = false

Function EnterDialogueWith(actor act)
	Sound bigTick = game.GetFormFromFile(0x000F38, "ORomancePlus.esp") as Sound
	bigTick.play(playerref)
	npc = act 
	firstOpen = true
	uiopen = true
...
Function ExitDialogue(int waitTime = 1)
	uiopen = false
	...
	;debug.Notification("exiting oromance dialogue")
	npc.EvaluatePackage()
...
function SayNo()
	int chance = 100
	int IR_Patch_loaded = Game.GetModByName("OR_ImmersiveRejections_Patch.esp")
	if IR_Patch_loaded != 255
	...
	if Utility.RandomInt(1, 100) > 35
		...
		if IR_Patch_loaded != 255
			GlobalVariable rejectGV = (game.GetFormFromFile(0x0F49, "ORomancePlus.esp") as GlobalVariable)
			rejectGV.SetValueInt(1)
			main.sayTopic(npc, main.marriagecourt)
			rejectGV.SetValueInt(0)
		endif
	endif 
endfunction
...
Event SuccessIndicatorThread(int type) 
	...
	if type == 0
		if (game.GetFormFromFile(0x0F46,"ORomancePlus.esp") as GlobalVariable).GetValueInt() == 0
			Return
		endif
		icon = "heart"
		color = colorPink
...
function SetAsFollower(actor act, bool set) ; follower in literal sense, not combat ally
	...
	Else
		if PlayerFollowerAlias.getReference() !=None
			bool cleared = false
			while (cleared != true)
				Utility.wait(0.2)
				if PlayerFollowerAlias.getReference() == None
				...
				cleared = PlayerFollowerAlias.TryToClear()
				;debug.notification("attempting to clear follower alias...")
			endwhile
		endif
...
function SetAsWaiting(actor act, bool set) ; follower in literal sense, not combat ally
	...
	Else
		...
		if WaitAlias.getReference() !=None
			bool cleared = false
			while (cleared != true)
				Utility.wait(0.2)
				if WaitAlias.getReference() == None
				...
				cleared = WaitAlias.TryToClear()
				;debug.notification("attempting to clear Wait alias...")
			endwhile
		endif
...
function Startup()  
	...
	;AmuletOfMara = game.getFormFromFile(0x0C891B, "Skyrim.esm") as Form
...
Function OnLoad()
	uiopen = false
	CacheRebuild = true
	SelectKey = (game.getFormFromFile(0x0F47, "ORomancePlus.esp") as GlobalVariable).GetValueInt()
...
Event OnKeyDown(Int KeyPress)
	...
	Sound smallTick = game.GetFormFromFile(0x000F36, "ORomancePlus.esp") as Sound
	Sound bigTick = game.GetFormFromFile(0x000F38, "ORomancePlus.esp") as Sound
	if !uiopen 
		return 
	endif
...
function safeForWorkMode(Actor npc)
	; Fade out over 2 seconds
	GlobalVariable currHour = game.GetFormFromFile(0x038,"skyrim.esm") as GlobalVariable
    Game.FadeOutGame(false, true, 2.0, 1.0)
    currHour.SetValue(currHour.GetValue() + 1)
    debug.notification(npc.GetDisplayName() + " enjoyed their time with you.")
    main.increaselovestat(npc, 1)
...
function ProcessEvent(int page, int element)
	...
	elseif page == PageStartSex
		...
		elseif element == 2
			...
			if prostitutionType == -1
				if (game.GetFormFromFile(0x0F54,"ORomancePlus.esp") as GlobalVariable).GetValueInt() == 0 || main.bridge == None
		...
		elseif element == 2
			if main.TrySeduce(npc) ;|| true 
				...
				if PlayerIsFollowed()
					...
					bool cheatPerk = playerref.hasPerk(Game.GetFormFromFile(0x000F32, "oromanceplus.esp") as perk)

					if main.getMonogamyDesireStat(npc) > 15 && !cheatPerk
						canThreesome = false 
					...
					if main.getMonogamyDesireStat(follower1) > 15 && !cheatPerk
						canThreesome = false 
	...
	elseif page == PageInteract
		...
		elseif element == 3
			...
			if !main.IsMarried(npc) && playerref.IsEquipped(AmuletOfMara)
				if main.TryPropose(npc)
					main.sayTopic(npc, main.marriagemara)
				endif
			endif
		elseif element == 4
			showpage(PageTalk)
			
		elseif element == 5 ;Gift interact
			ExitUI()
			SkyRomanceMiscFunction.SKSEGetNPCInventory(npc)
			int val = main.gift(npc)
			SkyRomanceMiscFunction.GetAddedItems(npc)
			...
			if !main.isPlayerPartner(npc)
				if main.TryAskOut(npc)
					...
					if npc.GetBaseObject().GetName() == "Serana"
						main.saytopic(npc, game.getFormFromFile(0x000824,"ORomancePlus.esp") as topic )
					else
						Utility.Wait(1)
						main.kiss(npc)
						ExitDialogue(1)
	...
	elseif page == PageApologize
		if element == 1
			ExitDialogue()
		elseif element == 2
			if main.canApologize(npc)
				main.TryApology(npc)
				clickElement(getelementbyid(element))
			Else
				debug.notification(npc.GetDisplayName() + " has heard enough.  Better luck tomorrow!")
				FireSuccessIncidcator(1)
			endif
		elseif element == 3
			int val = main.gift(npc)
			main.ProcessGift(npc, val, apology=true)
			clickElement(getelementbyid(element))
		EndIf
...
Function ShowPage(int pageID)
	...
	Elseif pageid == PageRomance
		int[] posData
		if prostitute
			posData = GetElementLayoutByElementCount(4)
		else
			posData = GetElementLayoutByElementCount(layoutCount)
		endif

		string kiss = "Try to kiss"
		string seduce = "Try to seduce"
		if PlayerIsFollowed()
	...
	elseif pageid == PageApologize
		int[] posData = GetElementLayoutByElementCount(3)
		Element1 = RenderElement(posData[0], "ecks", colorLightRed, "Exit")
		Element2 = RenderElement(posData[1], "anger", colorred, "Apologize")
		Element3 = RenderElement(posData[2], "gift", colorYellow, "Give gift")
	...
	Elseif pageid == PageInteract
		...
		Element4 = RenderElement(posData[3], "speech1", colorPink, "say")
		Element5 = RenderElement(posData[4], "gift", colorYellow, "Give gift")
	...
	Elseif pageid == PageRelationship
		int count = 4
		bool isAmuletEquipped = playerref.IsEquipped(AmuletOfMara)
		if Game.GetModByName("NPCs_Wear_Amulets_Of_Mara.esp") != 255
			isAmuletEquipped = isAmuletEquipped || npc.IsEquipped(AmuletOfMara)
		endif
		bool showMarry = !main.IsMarried(npc) && ( isAmuletEquipped ) && !main.isORMarriageLocked() && !main.isVanillaMarriageLocked()
		if !showMarry
			count = count - 2
		endIf
		bool showMarried = main.isPlayerSpouse(npc) && (!main.isPlayerVanillaSpouse(npc)) 
		bool showShopMoney =  npc.isinFaction(Game.GetFormFromFile(0x051596, "Skyrim.esm") as Faction) && (main.getStoreMoneyDiff(npc) >= 1)
		if (showMarried)
			count = 3
			if showShopMoney
				count = 4
		...
		if showMarry
			Element3 = RenderElement(posData[2], "question1", colorYellow, "Ask about marriage interest")
			Element4 = RenderElement(posData[3], "marry", colorYellow, "Propose marriage")
		endif
		if (showMarried)
			Element3 = RenderElement(posData[2], "heartring", colorRed, "Ask for homemade meal")
			if showShopMoney
				Element4 = RenderElement(posData[3], "coin", colorYellow, "Ask for store money")
			endif
		endif
	...
	Elseif pageid == PageTalk
		...
		Element1 = RenderElement(posData[0], "arrow", colorLightRed, "Back")
		Element2 = RenderElement(posData[1], "speech2", colorYellow, "Insult ")
		if main.canCompliment(npc)
			Element3 = RenderElement(posData[2], "speech1", colorGreen, "Compliment ")
		endif
	EndIf
...
Function UpdateColor()
	...
	elseif currentpage == PageRelationship
		...
		elseif selectedelement == 4 ;marry button
			if(!main.isPlayerSpouse(npc)) 
				int NPCSV = main.getmarrysv(npc)
				CalcColorBasedOnDiff(npcsv)
			else
...
int Function GetTextWidget(string widgettxt, int sizes)
	if PapyrusUtil.countString(TextWidgetkeys, widgettxt) > 0
		int z
		Int i = 0
...
int Function GetCoreWidget(string coreicon)
	if PapyrusUtil.countString(CoreWidgetkeys, coreicon) > 0
		int z
		Int i = 0
...
Function RebuildCacheSilent()
	if !orcache_locked
		...
		while orcache_locked
			utility.waitmenumode(0.1)
		endwhile
	endif 
	orcache_locked = false
	CacheRebuild = false
EndFunction
